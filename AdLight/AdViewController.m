...
- (IBAction)LightOn:(id)sender {
    ...
    [self lightRequest:[button tag]];
}


- (void) lightRequest:(int)num{
    if (!isBusy){ //灯空闲
        if (isServer) { //是Server就直接发送指令
            
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [self turnOnLight:num];
                NSLog(@"Finish light flow");
            });
        }
        else{ //if client
            [self sendAString:[NSString stringWithFormat:@"%d",num]];
        }  
    }
...
- (void)receiveData:(NSData *)data fromPeer:(NSString *)peer inSession:(GKSession *)session context:(void *)context
{
    ...
    if (isServer){
        int num;
        num = [receivedString intValue];
        [self lightRequest:num];
    }else{ //isClient
